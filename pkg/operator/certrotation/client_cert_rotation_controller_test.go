	...
	tests := []struct {
	...
	}{
		{
			...
			validateActions: func(t *testing.T, actions []clienttesting.Action) {
				// TODO[vrutkovs]: check actions in the order they are expected
				createCount := 0
				...
				if createCount < 3 {
					t.Errorf("expected at least 3 create actions, got %d", createCount)
				}
			},
		},
		{
		...
		},
		{
			...
			validateActions: func(t *testing.T, actions []clienttesting.Action) {
				...
				for _, action := range actions {
					...
					} else if action.GetVerb() == "create" && action.GetResource().Resource == "configmaps" {
						createAction := action.(clienttesting.CreateAction)
