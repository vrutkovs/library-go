...
type ImageTagMirrorSetLister interface {
	...
	List(ctx context.Context, selector labels.Selector) (ret []*configv1.ImageTagMirrorSet, err error)
	// Get retrieves the ImageTagMirrorSet from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(ctx context.Context, name string) (*configv1.ImageTagMirrorSet, error)
