...
type KubeSchedulerLister interface {
	...
	List(ctx context.Context, selector labels.Selector) (ret []*operatorv1.KubeScheduler, err error)
	// Get retrieves the KubeScheduler from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(ctx context.Context, name string) (*operatorv1.KubeScheduler, error)
