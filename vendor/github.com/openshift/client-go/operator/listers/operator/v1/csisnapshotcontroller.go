...
type CSISnapshotControllerLister interface {
	...
	List(ctx context.Context, selector labels.Selector) (ret []*operatorv1.CSISnapshotController, err error)
	// Get retrieves the CSISnapshotController from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(ctx context.Context, name string) (*operatorv1.CSISnapshotController, error)
