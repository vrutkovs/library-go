...
type HorizontalPodAutoscalerLister interface {
	...
	List(ctx context.Context, selector labels.Selector) (ret []*autoscalingv1.HorizontalPodAutoscaler, err error)
...
type HorizontalPodAutoscalerNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(ctx context.Context, name string) (*autoscalingv1.HorizontalPodAutoscaler, error)
