...
type HorizontalPodAutoscalerLister interface {
	...
	List(ctx context.Context, selector labels.Selector) (ret []*autoscalingv2.HorizontalPodAutoscaler, err error)
...
type HorizontalPodAutoscalerNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(ctx context.Context, name string) (*autoscalingv2.HorizontalPodAutoscaler, error)
