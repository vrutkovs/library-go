...
type ServiceCIDRLister interface {
	...
	List(ctx context.Context, selector labels.Selector) (ret []*networkingv1.ServiceCIDR, err error)
	// Get retrieves the ServiceCIDR from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(ctx context.Context, name string) (*networkingv1.ServiceCIDR, error)
